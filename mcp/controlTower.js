...
// The built-in MCP monitor doesn't use the LangChain callback, so we avoid
// pulling in @langchain/core here. asLangChainCallback() returns undefined.
const BaseCallbackHandler = null;
...
class ControlTower {
  ...
  asLangChainCallback() {
    ...
    if (!BaseCallbackHandler) {
      console.warn('[ControlTower] LangChain core not available — callback unavailable');
      return undefined;
    }
    // NOTE: synchronous on purpose. Callers do `callbacks: [ct.asLangChainCallback()]`
    // without awaiting, so this must return the handler itself (not a Promise).
