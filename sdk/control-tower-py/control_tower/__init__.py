...
class ControlTower:
    def __init__(
        self,
        agent_id:    str  = None,   # falls back to env CONTROL_TOWER_AGENT_ID
        server_url:  str  = None,   # falls back to env CONTROL_TOWER_URL
        name:        str  = None,
        ...
        agent_id   = agent_id or os.getenv("CONTROL_TOWER_AGENT_ID")
        if not agent_id:
            raise ValueError("agent_id is required (pass it or set CONTROL_TOWER_AGENT_ID)")
        server_url = server_url or os.getenv("CONTROL_TOWER_URL", "http://localhost:3090")
